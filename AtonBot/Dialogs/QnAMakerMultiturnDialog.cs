...
namespace MrBot.Dialogs
{
    ...
    public class QnAMakerMultiturnDialog : ComponentDialog
    {
        ...
        public const string DefaultNoAnswer = "Desculpe, não entendi. Sou um Bot!"; 

        // Card parameters	
        public const string DefaultCardTitle = "Uma destas opções lhe ajuda?";
        public const string DefaultCardNoMatchText = "Nenhuma destas alternativas.";
        public const string DefaultCardNoMatchResponse = "Obrigado pelo feedback.";
        ...
        public QnAMakerMultiturnDialog(IQnAMakerConfiguration configuration, MainMenuDialog mainMenuDialog) : base(nameof(QnAMakerMultiturnDialog))
        {
        ...
        private async Task<DialogTurnResult> CheckForMultiTurnPrompt(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            if (stepContext.Result is List<QueryResult> response && response.Count > 0)
            {
                ...
                if (answer.Context != null && answer.Context.Prompts != null && answer.Context.Prompts.Any())
                {
                    var dialogOptions = GetDialogOptionsValue(stepContext);
        ...
        private async Task<DialogTurnResult> DisplayQnAResult(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (reply != null && reply.Equals(qnaDialogResponseOptions.CardNoMatchText, StringComparison.OrdinalIgnoreCase))
            {
            ...
            else
            {
                ...
                // Se não está rodando sobre o MainMenu
                if ( ! Utility.DialogIsRunning(stepContext, nameof(MainMenuDialog)))
                    // Dispara o MainMenu para dar opções
                    return await stepContext.ReplaceDialogAsync(nameof(MainMenuDialog), dialogOptions, cancellationToken).ConfigureAwait(false);
            }
